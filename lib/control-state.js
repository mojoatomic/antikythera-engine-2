...
function effectiveDate(now = new Date()) {
  ...
  // Instrumentation: log effective date computation
  try {
    console.log('[control.effectiveDate] active:', ControlState.active, 'mode:', ControlState.mode);
    console.log('[control.effectiveDate] displayTime:', ControlState.displayTime, 'animate:', ControlState.animate);
    console.log('[control.effectiveDate] now:', now.toISOString(), '-> returning:', result.toISOString());
  } catch (_e) {}
  return result;
}
